...
void doska_drawFigura() // Вывод доски и фигур на экран
{
    system("cls"); // Очищение консоли
    char num = 'a'; // Для вывода указателя столбцов
    cout << "\t\t\t Ход: 'e2 e4' необходимо сделать пробел" << endl
       << "\t\t     (ход пешки с e2 на e4)";
    cout << "\n\n";
    for (int i = 0; i < 8; i++)
    {
        cout << "\t\t\t" << 8 - i;
...
void doska_def() {  // Стартовое положение доски и фигур
    Figura f;
    for (int i = 0; i < 8; i++)
    {   
        for (int j = 1; j < 9; j++)
    ...
    doska_drawFigura(); // Вывод на экран доску и фигуры
}
 
void doska_figura_delete(int x, int y) // Удаление фигуры из позиции
{
...
{           
    if (doska_b[8 - y][8 - (104 - x)] == 1) { // В области стоит фигура
        buff = doska[8 - y][8 - (104 - x)]; // поместить фигуру в буфер
        doska_figura_delete(x, y); // Удаление фигуры с доски
        doska_b[8 - y][8 - (104 - x)] = 0; // Состоянии улетки false
        ...
        doska_drawFigura();          // Выводим доску и фигуры
    }   
